@php
    $title = 'Brakes Adjustments - ' . ($vehicle->make?->name ?? 'Unknown') . ' ' . ($vehicle->model?->name ?? 'Unknown');
@endphp
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <div class="flex items-center gap-3 mb-2">
                ...
                <h1 class="text-2xl font-bold text-zinc-900 dark:text-zinc-100">Brakes Adjustments</h1>
            </div>
            <p class="text-zinc-600 dark:text-zinc-400">
                Brake system specifications and adjustment values for {{ $vehicle->make?->name ?? 'Unknown' }} {{ $vehicle->model?->name ?? 'Unknown' }} ({{ $vehicle->registration }})
            </p>
        </div>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-sm text-red-800 dark:text-red-200">
                            <strong>Error loading brakes data:</strong> {{ $error }}
                        </p>
                    </div>
                </div>
            </div>
                        ...
                        <div class="p-6">
                            @if(isset($section['subAdjustments']) && !empty($section['subAdjustments']))
                                ...
                                @foreach($section['subAdjustments'] as $subsection)
                                    <div class="mb-6 @if(!$loop->first) mt-8 pt-6 border-t border-zinc-200 dark:border-zinc-700 @endif">
                                        <h3 class="text-md font-medium text-zinc-800 dark:text-zinc-200 mb-4">
                                            {{ $subsection['name'] }}
                                            @if($subsection['remark'])
                                                <span class="text-sm text-zinc-500 font-normal ml-2">({{ $subsection['remark'] }})</span>
                                            @endif
                                        </h3>
                                        
                                        @if(isset($subsection['subAdjustments']) && !empty($subsection['subAdjustments']))
                                            <div class="overflow-x-auto">
                                                <table class="w-full">
                                                    <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                                                        @foreach($subsection['subAdjustments'] as $item)
                                                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                                                                <td class="px-4 py-3 text-sm font-medium text-zinc-900 dark:text-zinc-100 w-1/2">
                                                                    {{ $item['name'] }}
                                                                    @if($item['remark'])
                                                                        <span class="text-sm text-zinc-500 font-normal ml-2">({{ $item['remark'] }})</span>
                                                                    @endif
                                                                </td>
                                                                    ...
                                                                    <div class="flex items-start gap-4">
                                                                        <div class="flex-1">
                                                                            @if($item['value'])
                                                                                ...
                                                                                @if($item['unit'])
                                                                                    <span class="text-zinc-500 ml-1">{{ $item['unit'] }}</span>
                                                                                @endif
                                                                            @else
                                                                            ...
                                                                            <div class="flex-shrink-0">
                                                                                <div class="bg-zinc-50 dark:bg-zinc-800 rounded-lg p-2 border border-zinc-200 dark:border-zinc-700">
                                                                                    <img 
                                                                                        ...
                                                                                        onclick="openImageModal('{{ addslashes($item['imageName']) }}', '{{ addslashes($item['name']) }}')"
                                                                                    >
                                                            ...
                                                            <!-- Handle nested sub-specifications ONLY for rear brakes -->
                                                            @if(isset($item['subAdjustments']) && !empty($item['subAdjustments']))
                                                                @foreach($item['subAdjustments'] as $spec)
                                                                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 bg-zinc-50/50 dark:bg-zinc-800/25">
                                                                        <td class="px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300 w-1/2 pl-8">
                                                                            {{ $spec['name'] }}
                                                                            @if($spec['remark'])
        ...
        @else
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                <div class="p-8 text-center">
                    ...
                    <p class="text-zinc-600 dark:text-zinc-400">
                        Brake system data could not be loaded from the HaynesPro API for this vehicle.
                    </p>
                </div>
            </div>
        @endif
            ...
            <div class="flex items-start gap-3">
                ...
                <div>
                    <p class="text-sm text-blue-800 dark:text-blue-200">
                        @if(!empty($brakesData))
                            <strong>Live Data:</strong> Brake system specifications are dynamically fetched from the HaynesPro API 
                            based on this vehicle's car type ID ({{ $vehicle->car_type_id ?? 'Not available' }}). Data includes disc dimensions, pad thicknesses, and system pressures for different vehicle variants.
                        @else
                            <strong>No Data Available:</strong> Brake system data could not be loaded from the HaynesPro API. 
        ...
        @if(!empty($brakesData))
            <div class="mt-6 bg-zinc-50 dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                <h3 class="text-sm font-medium text-zinc-900 dark:text-zinc-100 mb-3">Brake System Terms</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 text-xs">
                    <div class="flex gap-2">
                        ...
                        <span class="text-zinc-600 dark:text-zinc-400">Millimeters (dimension)</span>
                    </div>
                    <div class="flex gap-2">
                        <span class="font-medium text-zinc-700 dark:text-zinc-300">bar:</span>
                        <span class="text-zinc-600 dark:text-zinc-400">Unit of pressure</span>
                    </div>
                    <div class="flex gap-2">
                        <span class="font-medium text-zinc-700 dark:text-zinc-300">Disc run-out:</span>
                        <span class="text-zinc-600 dark:text-zinc-400">Maximum disc warpage</span>
                    </div>
                    <div class="flex gap-2">
                        <span class="font-medium text-zinc-700 dark:text-zinc-300">Ventilated:</span>
                        <span class="text-zinc-600 dark:text-zinc-400">Disc with cooling vanes</span>
                    </div>
                    <div class="flex gap-2">
                        <span class="font-medium text-zinc-700 dark:text-zinc-300">90/110/130:</span>
                        <span class="text-zinc-600 dark:text-zinc-400">Vehicle model variants</span>
                    </div>
                    <div class="flex gap-2">
                        <span class="font-medium text-zinc-700 dark:text-zinc-300">Pad thickness:</span>
                        <span class="text-zinc-600 dark:text-zinc-400">Brake pad wear limit</span>
                    </div>
                </div>
            </div>
        @endif
